<template>
  <v-container fluid>
    <material-cardy color="#283E4A" ref="normal" title="Ongoing Return-Parcels">
      <v-data-table :headers="headers" :items="ongoing_return" :search="searcher">
    ...
    <material-cardy color="#283E4A" ref="normal" title="Received Return-Parcels">
      <v-data-table :headers="headers" :items="returned_v" :search="searcher">
        <template v-slot:body="{items}">
          <tbody>
            <tr v-show="items.length == 0">
              <td colspan="11" class="font-weight-bold">
...
export default {
  ...
  async asyncData(context) {
    ...
    console.log(data);
    let vouchers = data;
    context.store.commit("title/setTitle", "Return Parcels");
    ...
    return { returned_v, ongoing_return };
  },
  data() {
    return {
      headers: [
